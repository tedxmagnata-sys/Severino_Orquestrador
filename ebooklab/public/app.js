...
const TOKEN_KEY = 'lb_access_token';
function getToken() { return localStorage.getItem(TOKEN_KEY) || ''; }
function setToken(t) { if (t) localStorage.setItem(TOKEN_KEY, t); else localStorage.removeItem(TOKEN_KEY); }

let resultadoAtual = null;
let cfg = { free_limit_dia: 3, checkout_url: '#', produto: 'EbookLab Engine' };
...
async function api(url, opts = {}) {
  ...
  if (!res.ok) throw Object.assign(new Error(data.erro || data.error || ('HTTP ' + res.status)), { status: res.status, data });
  return data;
}

function renderPlano(plano, usados, limite) {
  const badge = $('#planoBadge');
  if (plano === 'pro') {
    badge.textContent = (limite > 0) ? `Pro · ${usados}/${limite}` : 'Pro';
    badge.classList.add('pro');
  } else {
    badge.textContent = `Grátis · ${usados}/${limite}`;
    badge.classList.remove('pro');
  }
}

function abrirModal(pro = false) {
  $('#mFreeLimite').textContent = cfg.free_limit_dia;
  $('#btnCompra').href = cfg.checkout_url || '#';
  $('#btnCompra').classList.remove('oculto');
  $('#btnCompra').style.display = '';
  const msg = $('#unlockMsg');
  msg.classList.add('oculto');
  if (pro) {
    $('#btnCompra').style.display = 'none';
    document.querySelector('#modalPlanos .sub').textContent = 'Você atingiu o limite diário do plano Pro (uso justo). O contador zera amanhã automaticamente.';
  }
  $('#modalPlanos').classList.remove('oculto');
}
...
async function checarSaude() {
  try {
    ...
    renderPlano(r.plano && r.plano.plano, r.usados_dia, r.limite_dia);
  } catch {
...
async function checarSaude() {
  try {
    ...
    $('#statusTxt').textContent = 'online';
    if (!r.tem_capa) $('#capaInfo').textContent = 'prompt (sem chave de imagem)';
    renderPlano(r.plano && r.plano.plano, r.free_usados, r.free_limit_dia);
  } catch {
    $('#statusTxt').textContent = 'offline';
  }
}
...
async function gerar() {
  ...
  try {
    ...
    if (e.status === 402) {
      abrirModal(e.data.plano === 'pro');
      renderPlano(e.data.plano === 'pro' ? 'pro' : 'free', cfg.usados_dia ?? 0, cfg.limite_dia ?? cfg.free_limit_dia);
    } else {
...
$('#modalFechar').addEventListener('click', () => $('#modalPlanos').classList.add('oculto'));
$('#modalPlanos').addEventListener('click', (e) => { if (e.target === e.currentTarget) $('#modalPlanos').classList.add('oculto'); });

$('#btnUnlock').addEventListener('click', async () => {
  const email = $('#unlockEmail').value.trim();
  const msg = $('#unlockMsg');
  msg.classList.remove('oculto', 'erro');
  if (!email) { msg.textContent = 'Digite o e-mail usado na compra.'; return; }
  msg.textContent = 'Verificando…';
  try {
    const r = await api('/api/unlock', { method: 'POST', body: JSON.stringify({ email }) });
    setToken(r.token);
    msg.textContent = 'Acesso Pro liberado! Recarregando…';
