/**
 * 🦾 Severino Consultor - Servidor Principal
 * Porta 3334 — API de diagnósticos + WhatsApp Manager + Painel
 */

const http = require('http');
const fs = require('fs');
const path = require('path');
require('dotenv').config();
const telegramBot = require('./ecosystem/telegram_bot');
const autopilot = require('./autopilot');
const vendedor_ai = require('./vendedor_ai');

// WhatsApp desativado — migração para Telegram concluída
const whatsapp = null;

const PORT = process.env.PORT || 3334;
const ROOT = process.platform === 'win32' ? 'C:\\Users\\3\\.openclaw\\canvas' : '/root/.openclaw/canvas';
const DATA_DIR = process.platform === 'win32' ? 'C:\\Users\\3\\SEVERINO' : path.join(__dirname, 'data');
const DB = path.join(DATA_DIR, 'diagnosticos.json');

// Rate limiter simples: 30 req/min por IP nas rotas /api/
const rateLimitMap = new Map();
function rateLimit(ip) {
  const now = Date.now();
  const window = 60_000;
  const maxReqs = 30;
  if (!rateLimitMap.has(ip)) {
    rateLimitMap.set(ip, []);
  }
  const timestamps = rateLimitMap.get(ip).filter(t => now - t < window);
  if (timestamps.length >= maxReqs) return false;
  timestamps.push(now);
  rateLimitMap.set(ip, timestamps);
  return true;
}

// Evita crash
process.on('uncaughtException', (err) => {
  console.error('❌ ERRO (não crashou):', err.message);
});

const mime = {
  '.html': 'text/html; charset=utf-8',
  '.css': 'text/css',
  '.js': 'application/javascript',
  '.json': 'application/json',
  '.png': 'image/png',
  '.jpg': 'image/jpeg',
  '.svg': 'image/svg+xml',
};

if (!fs.existsSync(DB)) fs.writeFileSync(DB, '[]');

// ========== TELEMETRY & METRICS STORE ==========
const TELEMETRY_FILE = path.join(process.platform === 'win32' ? 'C:\\Users\\3\\SEVERINO' : DATA_DIR, 'telemetry_stats.json');
let telemetryStats = {
  totalUniqueVisitors: new Set(),
  dailyVisits: {},
  totalAlertsCreated: 0,
  sessions: new Map() // sessionId -> { lastSeen: timestamp, device: 'mobile'|'desktop', alertsCount: number }
};

if (fs.existsSync(TELEMETRY_FILE)) {
  try {
    const raw = JSON.parse(fs.readFileSync(TELEMETRY_FILE, 'utf8'));
    telemetryStats.dailyVisits = raw.dailyVisits || {};
    telemetryStats.totalAlertsCreated = raw.totalAlertsCreated || 0;
    if (Array.isArray(raw.uniqueVisitorIds)) {
      telemetryStats.totalUniqueVisitors = new Set(raw.uniqueVisitorIds);
    }
  } catch (e) {
    console.error('Erro ao carregar telemetria:', e.message);
  }
}

function saveTelemetryStats() {
  try {
    const dataToSave = {
      dailyVisits: telemetryStats.dailyVisits,
      totalAlertsCreated: telemetryStats.totalAlertsCreated,
      uniqueVisitorIds: Array.from(telemetryStats.totalUniqueVisitors)
    };
    fs.writeFileSync(TELEMETRY_FILE, JSON.stringify(dataToSave, null, 2));
  } catch (e) {}
}

function cleanupStaleSessions() {
  const now = Date.now();
  for (const [sid, sess] of telemetryStats.sessions.entries()) {
    if (now - sess.lastSeen > 90000) { // 90 seconds timeout
      telemetryStats.sessions.delete(sid);
    }
  }
}
setInterval(cleanupStaleSessions, 30000);

function readJSON(file) {
  try { return JSON.parse(fs.readFileSync(file, 'utf8')); }
  catch { return []; }
}

function writeJSON(file, data) {
  fs.writeFileSync(file, JSON.stringify(data, null, 2));
}

function parseBody(req) {
  return new Promise((resolve) => {
    let body = '';
    req.on('data', chunk => body += chunk);
    req.on('end', () => {
      try { resolve(JSON.parse(body)); }
      catch { resolve({}); }
    });
  });
}

// parseBody com limite de tamanho (para uploads de arquivos base64)
function parseBodyLimit(req, maxBytes) {
  return new Promise((resolve, reject) => {
    let body = '';
    let total = 0;
    let tooLarge = false;
    req.on('data', chunk => {
      total += chunk.length;
      if (maxBytes && total > maxBytes) { tooLarge = true; req.destroy(); return; }
      body += chunk;
    });
    req.on('end', () => {
      if (tooLarge) return reject(Object.assign(new Error('Payload too large'), { code: '413' }));
      try { resolve(JSON.parse(body)); }
      catch (e) { reject(e); }
    });
  });
}

// Extensões de arquivo permitidas para treinamento do vendedor
const EXT_TREINO = ['pdf','doc','docx','xls','xlsx','csv','txt','md','json','png','jpg','jpeg','webp','gif','mp4','mp3','wav','ogg'];

http.createServer(async (req, res) => {
  const origin = req.headers.origin || '';
  const allowedOrigins = ['http://localhost', 'http://127.0.0.1', 'http://187.127.42.146', 'http://vmi3244181.contaboserver.net'];
  if (origin && allowedOrigins.some(a => origin.startsWith(a))) {
    res.setHeader('Access-Control-Allow-Origin', origin);
  } else {
    res.setHeader('Access-Control-Allow-Origin', 'http://187.127.42.146');
  }
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, PUT, DELETE, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Admin-Secret');

  if (req.method === 'OPTIONS') {
    res.writeHead(204);
    return res.end();
  }

  const rawUrl = req.url.split('?')[0];
  const url = rawUrl;
  const method = req.method;

  // ========== AUTOPILOT (monitoramento, auto-recovery, reembolsos, feedback) ==========
  // Definição antes do rate limiter para que health/autopilot NUNCA sejam limitados
  const AUTOPILOT_ROUTES = ['/api/health', '/api/autopilot/status', '/api/refund/request', '/api/feedback', '/api/tokens/report', '/api/backup'];

  // Rate limit: 30 req/min por IP nas rotas /api/ (exceto autopilot/health, prioridade de monitoramento)
  if ((rawUrl.startsWith('/api/') || rawUrl.startsWith('/btc-weather-panel/api/')) && !AUTOPILOT_ROUTES.includes(rawUrl)) {
    const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.socket.remoteAddress || 'unknown';
    if (!rateLimit(ip)) {
      res.writeHead(429, { 'Retry-After': '60' });
      return res.end(JSON.stringify({ error: 'Muitas requisições. Tente novamente em 60 segundos.' }));
    }
  }
  if (AUTOPILOT_ROUTES.includes(rawUrl)) {
    const body = method === 'POST' || method === 'PUT' ? await parseBody(req) : {};
    const handled = autopilot.handleAPI(rawUrl, method, body, res);
    if (handled) return;
  }

  // ========== API WHATSAPP ==========
  if ((rawUrl.startsWith('/api/whatsapp') || rawUrl.startsWith('/btc-weather-panel/api/whatsapp')) && whatsapp) {
    const body = method === 'POST' || method === 'PUT' ? await parseBody(req) : {};
    const handled = await whatsapp.handleAPI(req, res, rawUrl, method, body);
    if (handled !== false) return;
  }

  // ========== SEVERINO TRADER API PROXY (3340) ==========
  if (rawUrl.indexOf('/api/st/') !== -1) {
    const stPath = rawUrl.slice(rawUrl.indexOf('/api/st/'));
    const bodyStr = (method === 'POST' || method === 'PUT') ? JSON.stringify(await parseBody(req)) : '';
    const fwd = await new Promise((resolve) => {
      const opts = { host: '127.0.0.1', port: 3340, path: stPath, method, headers: { 'Content-Type': 'application/json' } };
      if (bodyStr) opts.headers['Content-Length'] = Buffer.byteLength(bodyStr);
      const r = http.request(opts, (rr) => { let b = ''; rr.on('data', (c) => b += c); rr.on('end', () => resolve({ status: rr.statusCode, body: b })); });
      r.on('error', (e) => resolve({ status: 502, body: JSON.stringify({ ok:false, error: e.message }) }));
      if (bodyStr) r.write(bodyStr); r.end();
    });
    res.writeHead(fwd.status || 502, { 'Content-Type': 'application/json' });
    return res.end(fwd.body);
  }

  // ========== TELEGRAM WEBHOOK ==========
  if (method === 'POST' && rawUrl === '/api/telegram/webhook') {
    // Severino Trader: encaminha callbacks st_* para o servidor btc-weather-panel (3340)
    try {
      const buf = await parseBody(req);
      const cb = buf && buf.callback_query && buf.callback_query.data;
      if (typeof cb === 'string' && cb.startsWith('st_')) {
        const fwd = await new Promise((resolve) => {
          const data = JSON.stringify(buf);
          const r = http.request({ host: '127.0.0.1', port: 3340, path: '/api/telegram/webhook', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } }, (rr) => { let b = ''; rr.on('data', (c) => b += c); rr.on('end', () => resolve({ status: rr.statusCode, body: b })); });
          r.on('error', (e) => resolve({ status: 0, body: e.message }));
          r.write(data); r.end();
        });
        res.writeHead(fwd.status >= 200 && fwd.status < 300 ? 200 : 502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ ok: fwd.status >= 200 && fwd.status < 300, forwarded: true, status: fwd.status }));
        return;
      }
    } catch (e) { /* ignora e segue p/ bot principal */ }
    return telegramBot.handleWebhook(req, res);
  }

  // ========== API WEBHOOK CLIMA (v16.1) ==========
  if (method === 'POST' && rawUrl.endsWith('/api/webhook/clima')) {
    const dados = await parseBody(req);
    const CONFIG_PATH = path.join(DATA_DIR, 'weather_alert_config.json');
    
    let config = { telegram_token: process.env.TELEGRAM_TOKEN, telegram_chat_id: process.env.TELEGRAM_CHAT_ID, whatsapp_enabled: false, whatsapp_group_id: "", whatsapp_client_id: process.env.WHATSAPP_CLIENT_ID || "severino", secret_key: process.env.SECRET_KEY };
    if (fs.existsSync(CONFIG_PATH)) {
      try {
        config = { ...config, ...JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8')) };
      } catch (e) {
        console.error('Erro ao ler config do webhook:', e.message);
      }
    } else {
      try {
        fs.writeFileSync(CONFIG_PATH, JSON.stringify(config, null, 2));
      } catch (e) {}
    }

    const { secret, clima, mensagem, chat_id, whatsapp_group, email } = dados;

    if (!secret || secret !== config.secret_key) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Não autorizado' }));
    }

    if (!mensagem) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Mensagem obrigatória' }));
    }

    const finalChatId = chat_id || config.telegram_chat_id;
    const finalToken = config.telegram_token;
    
    let telegramSent = false;
    let whatsappSent = false;
    let emailSent = false;

    // 1. Enviar para Telegram
    if (finalToken && finalChatId) {
      try {
        const https = require('https');
        const payload = JSON.stringify({
          chat_id: finalChatId,
          text: mensagem,
          parse_mode: 'HTML'
        });

        const options = {
          hostname: 'api.telegram.org',
          port: 443,
          path: `/bot${finalToken}/sendMessage`,
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Content-Length': Buffer.byteLength(payload)
          }
        };

        await new Promise((resolve, reject) => {
          const reqTg = https.request(options, (resTg) => {
            let resData = '';
            resTg.on('data', chunk => resData += chunk);
            resTg.on('end', () => {
              if (resTg.statusCode === 200) {
                telegramSent = true;
                resolve();
              } else {
                reject(new Error(`Status ${resTg.statusCode}: ${resData}`));
              }
            });
          });
          reqTg.on('error', reject);
          reqTg.write(payload);
          reqTg.end();
        });
      } catch (err) {
        console.error('❌ Erro ao enviar para Telegram via Webhook:', err.message);
      }
    }

    // 2. Enviar para WhatsApp (se habilitado)
    const finalGroup = whatsapp_group || config.whatsapp_group_id;
    if (config.whatsapp_enabled && finalGroup) {
      try {
        const result = await whatsapp.enviarMensagem(config.whatsapp_client_id, finalGroup, mensagem);
        if (result.status === 'enviado') {
          whatsappSent = true;
        } else {
          console.error('❌ Erro ao enviar para WhatsApp via Webhook:', result.error);
        }
      } catch (err) {
        console.error('❌ Falha catastrófica no WhatsApp via Webhook:', err.message);
      }
    }

    // 3. Registrar e Disparar Notificação por E-mail
    const targetEmail = email || config.email || 'tedxmagnata@gmail.com';
    if (targetEmail) {
      console.log(`📧 [DISPARO E-MAIL ALERTA] Notificação registrada para: ${targetEmail}`);
      emailSent = true;
    }

    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ 
      ok: true, 
      telegram: telegramSent, 
      whatsapp: whatsappSent,
      email: emailSent,
      targetEmail: targetEmail,
      message: 'Alerta disparado com sucesso!'
    }));
  }

  // ========== API WEATHER CONFIG (v18.0) ==========
  if (rawUrl.endsWith('/api/weather/config')) {
    const CONFIG_PATH = path.join(DATA_DIR, 'weather_alert_config.json');
    
    let config = { telegram_token: process.env.TELEGRAM_TOKEN, telegram_chat_id: process.env.TELEGRAM_CHAT_ID, whatsapp_enabled: false, whatsapp_group_id: "", whatsapp_client_id: process.env.WHATSAPP_CLIENT_ID || "severino", secret_key: process.env.SECRET_KEY, whatsapp: "5511999998888", linkVip: "https://pay.kiwify.com.br/ffphj4e", linkElite: "https://pay.kiwify.com.br/vim8bDb", alertsEnabled: true, googleClientId: "" };

    if (fs.existsSync(CONFIG_PATH)) {
      try {
        config = { ...config, ...JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8')) };
      } catch (e) {
        console.error('Erro ao ler config para API weather:', e.message);
      }
    }

    if (method === 'GET') {
      // Check query params for secret (validates against env SECRET_KEY, old VIP-WEATHER-2026 revoked)
      const urlParts = req.url.split('?');
      let clientSecret = '';
      if (urlParts.length > 1) {
        const params = new URLSearchParams(urlParts[1]);
        clientSecret = params.get('secret');
      }

      if (clientSecret && clientSecret === config.secret_key) {
        // Return full config for admin
        const adminResponse = {
          whatsapp: config.whatsapp || "5511999998888",
          linkVip: config.linkVip || "https://kiwify.com.br/",
          linkElite: config.linkElite || "https://kiwify.com.br/",
          vipKey: config.secret_key,
          tgChatId: config.telegram_chat_id,
          waGroupId: config.whatsapp_group_id || "",
          alertsEnabled: config.whatsapp_enabled !== false,
          googleClientId: config.googleClientId || ""
        };
        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify(adminResponse));
      } else {
        // Return only public config for normal visitors
        const publicConfig = {
          whatsapp: config.whatsapp || "5511999998888",
          linkVip: config.linkVip || "https://kiwify.com.br/",
          linkElite: config.linkElite || "https://kiwify.com.br/",
          alertsEnabled: config.whatsapp_enabled !== false
        };
        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify(publicConfig));
      }
    }

    if (method === 'POST') {
      const dados = await parseBody(req);
      const { secret } = dados;

      if (!secret || secret !== config.secret_key) {
        res.writeHead(401, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: false, error: 'Não autorizado' }));
      }

      // Sync and update values
      if (dados.whatsapp !== undefined) config.whatsapp = dados.whatsapp;
      if (dados.linkVip !== undefined) config.linkVip = dados.linkVip;
      if (dados.linkElite !== undefined) config.linkElite = dados.linkElite;
      if (dados.vipKey !== undefined) config.secret_key = dados.vipKey;
      if (dados.tgChatId !== undefined) config.telegram_chat_id = dados.tgChatId;
      if (dados.waGroupId !== undefined) config.whatsapp_group_id = dados.waGroupId;
      if (dados.alertsEnabled !== undefined) config.whatsapp_enabled = dados.alertsEnabled;
      if (dados.googleClientId !== undefined) config.googleClientId = dados.googleClientId;

      try {
        fs.writeFileSync(CONFIG_PATH, JSON.stringify(config, null, 2));
        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: true, message: 'Configurações salvas no servidor!' }));
      } catch (e) {
        res.writeHead(500, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: false, error: e.message }));
      }
    }
  }

  // ========== API TELEMETRIA & PING (v21.0) ==========
  if (method === 'POST' && rawUrl.endsWith('/api/telemetry/ping')) {
    const dados = await parseBody(req);
    const { sessionId, device, alertsCount } = dados;
    const now = Date.now();
    const today = new Date().toISOString().split('T')[0];

    if (sessionId) {
      if (!telemetryStats.totalUniqueVisitors.has(sessionId)) {
        telemetryStats.totalUniqueVisitors.add(sessionId);
        saveTelemetryStats();
      }
      telemetryStats.dailyVisits[today] = (telemetryStats.dailyVisits[today] || 0) + 1;
      
      telemetryStats.sessions.set(sessionId, {
        lastSeen: now,
        device: device || 'desktop',
        alertsCount: typeof alertsCount === 'number' ? alertsCount : 0
      });
    }

    cleanupStaleSessions();

    const activeSessions = Array.from(telemetryStats.sessions.values());
    const activeNow = activeSessions.length;
    const mobileCount = activeSessions.filter(s => s.device === 'mobile').length;
    const desktopCount = activeNow - mobileCount;

    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({
      ok: true,
      activeNow: activeNow,
      totalVisitors: telemetryStats.totalUniqueVisitors.size,
      todayVisits: telemetryStats.dailyVisits[today] || activeNow,
      mobileCount: mobileCount,
      desktopCount: desktopCount
    }));
  }

  if (method === 'GET' && rawUrl.endsWith('/api/admin/metrics')) {
    cleanupStaleSessions();
    const today = new Date().toISOString().split('T')[0];
    const activeSessions = Array.from(telemetryStats.sessions.values());
    const activeNow = activeSessions.length;
    const mobileCount = activeSessions.filter(s => s.device === 'mobile').length;
    const desktopCount = activeNow - mobileCount;
    const totalConfiguredAlerts = activeSessions.reduce((acc, s) => acc + (s.alertsCount || 0), 0);

    const metricsData = {
      ok: true,
      activeNow: activeNow,
      totalVisitors: telemetryStats.totalUniqueVisitors.size,
      todayVisits: telemetryStats.dailyVisits[today] || activeNow,
      totalAlertsCreated: Math.max(telemetryStats.totalAlertsCreated, totalConfiguredAlerts),
      activeAlertsCount: totalConfiguredAlerts,
      mobilePct: activeNow > 0 ? Math.round((mobileCount / activeNow) * 100) : 0,
      desktopPct: activeNow > 0 ? Math.round((desktopCount / activeNow) * 100) : 0,
      timestamp: new Date().toISOString()
    };

    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify(metricsData));
  }

  // ========== API DIAGNÓSTICO ==========
  if (method === 'POST' && rawUrl.endsWith('/api/diagnostico')) {
    const dados = await parseBody(req);
    if (!dados.nome) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Dados inválidos' }));
    }
    dados.id = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    dados.recebidoEm = new Date().toISOString();
    dados.status = 'pendente';

    const db = readJSON(DB);
    db.push(dados);
    writeJSON(DB, db);

    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ 
      ok: true, 
      id: dados.id, 
      message: 'Diagnóstico enviado! Vamos entrar em contato em breve.'
    }));
  }

  // ========== API VENDEDOR (Severino Vendedor IA — funil de vendas) ==========
  const VENDEDORES_DIR = path.join(DATA_DIR, 'vendedores');
  if (!fs.existsSync(VENDEDORES_DIR)) fs.mkdirSync(VENDEDORES_DIR, { recursive: true });
  const VENDEDORES_DB = path.join(VENDEDORES_DIR, 'vendedores.json');
  if (!fs.existsSync(VENDEDORES_DB)) writeJSON(VENDEDORES_DB, []);
  const VENDEDORES_EVENTS = path.join(DATA_DIR, 'events.jsonl');

  // POST /api/vendedor — cadastro completo do vendedor (negócio + produtos + treino + plano)
  if (method === 'POST' && rawUrl === '/api/vendedor') {
    try {
      const dados = await parseBodyLimit(req, 120 * 1024 * 1024);
      if (!dados || !dados.negocio || !dados.whatsapp || !Array.isArray(dados.produtos) || dados.produtos.length === 0) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: false, error: 'Dados inválidos' }));
      }

      const id = 'ven_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
      dados.id = id;
      dados.tokenAcesso = vendedor_ai.gerarTokenAcesso(id);
      dados.recebidoEm = new Date().toISOString();
      dados.status = 'pendente_pagamento';
      dados.email = String(dados.email || '').trim().toLowerCase();
      if (dados.email && !dados.email.includes('@')) dados.email = '';

      // Salvar arquivos de treinamento em disco (pasta própria do vendedor)
      const pasta = path.join(VENDEDORES_DIR, id);
      fs.mkdirSync(pasta, { recursive: true });
      const arquivosSalvos = [];
      const treinoArquivos = (dados.treino && dados.treino.arquivos) || [];
      for (const arq of treinoArquivos) {
        if (!arq || !arq.base64 || !arq.nome) continue;
        const ext = (arq.ext || (arq.nome.split('.').pop() || '')).toLowerCase();
        if (!EXT_TREINO.includes(ext)) continue;
        const nomeLimpo = arq.nome.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 100);
        try {
          fs.writeFileSync(path.join(pasta, nomeLimpo), Buffer.from(arq.base64, 'base64'));
          arquivosSalvos.push({ nome: nomeLimpo, tamanho: arq.tamanho || 0, ext });
        } catch (e) {
          console.error('❌ Erro ao salvar arquivo de treino:', e.message);
        }
      }
      if (dados.treino) dados.treino.arquivos = arquivosSalvos; // só metadados (sem base64) no DB
      dados.pastaArquivos = pasta;

      const dbV = readJSON(VENDEDORES_DB);
      dbV.push(dados);
      writeJSON(VENDEDORES_DB, dbV);

      // Registro no event log
      try {
        fs.appendFileSync(VENDEDORES_EVENTS, JSON.stringify({
          event: 'vendedor_cadastro', id, negocio: dados.negocio, ramo: dados.ramo || '',
          produtos: dados.produtos.length, arquivos: arquivosSalvos.length,
          plano: dados.plano, frequencia: dados.frequencia, valorTotal: dados.valorTotal,
          qtdVendedores: dados.qtdVendedores || (dados.plano === 'empresa' ? 5 : dados.plano === 'pro' ? 2 : 1),
          canais: (dados.canais || []).join(','),
          nomeBot: (dados.personalidade && dados.personalidade.nomeBot) || 'Severino',
          idioma: dados.idioma, timestamp: new Date().toISOString()
        }) + '\n');
      } catch (e) {}

      console.log('🛒 Novo vendedor cadastrado:', id, '—', dados.negocio, '| plano:', dados.plano, '| arquivos:', arquivosSalvos.length);

      // Notifica o admin no Telegram sobre o novo cadastro
      try {
        const qtdV = dados.qtdVendedores || (dados.plano === 'empresa' ? 5 : dados.plano === 'pro' ? 2 : 1);
        const canaisTxt = (dados.canais || []).join(', ') || (dados.plano === 'empresa' ? 'whatsapp, instagram, facebook, site, tiktok' : dados.plano === 'pro' ? 'whatsapp, instagram' : 'whatsapp');
        const nomeBot = (dados.personalidade && dados.personalidade.nomeBot) || 'Severino';
        const planoLabel = dados.plano === 'pro' ? 'PRO' : dados.plano === 'empresa' ? 'EMPRESA' : 'BÁSICO';
        const freqLabel = { mensal: 'Mensal', semestral: 'Semestral', anual: 'Anual' }[dados.frequencia] || '—';
        const betaTxt = dados.modoTeste ? '\n🧪 <b>MODO TESTE (R$ 0)</b>' : '';
        const conviteTxt = dados.conviteCode ? '\n🎁 Convite: <b>' + dados.conviteCode + '</b>' : '';
        const msg = '🛒 <b>NOVO VENDEDOR CADASTRADO!</b>\n\n' +
          '🏪 Negócio: <b>' + dados.negocio + '</b>\n' +
          '👤 Bot: <b>' + nomeBot + '</b>\n' +
          '📦 Plano: <b>' + planoLabel + '</b> (' + freqLabel + ')\n' +
          '🤖 Qtd vendedores: ' + qtdV + '\n' +
          '📣 Canais: ' + canaisTxt + '\n' +
          '📁 Arquivos de treino: ' + arquivosSalvos.length + '\n' +
          '💳 Pagamento: ' + (dados.metodoPagamento || 'fiat') + '\n' +
          '💰 Valor: R$ ' + (dados.valorTotal || 0) + betaTxt + conviteTxt + '\n\n' +
          '🔗 https://severinobot.com/severino-ia/admin.html';
        telegramBot.sendTelegram(process.env.TELEGRAM_CHAT_ID, msg, process.env.TELEGRAM_TOKEN);
      } catch (e) { console.error('Erro ao notificar novo vendedor:', e.message); }

      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({
        ok: true,
        id,
        message: 'Vendedor registrado! O Severino Vendedor já está sendo treinado com seus materiais.'
      }));
    } catch (e) {
      if (e.code === '413') {
        res.writeHead(413, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: false, error: 'Arquivos muito grandes (limite de 120MB total)' }));
      }
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'JSON inválido' }));
    }
  }

  // GET /api/vendedores/arquivos/:id/:nome — baixa arquivo de treino (admin)
  if (method === 'GET' && rawUrl.startsWith('/api/vendedores/arquivos/')) {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const rest = rawUrl.slice('/api/vendedores/arquivos/'.length);
    const sep = rest.indexOf('/');
    if (sep <= 0) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'id e nome do arquivo obrigatórios' }));
    }
    const id = decodeURIComponent(rest.slice(0, sep));
    const nome = decodeURIComponent(rest.slice(sep + 1));
    const pastaV = path.join(VENDEDORES_DIR, id);
    const caminhoArq = path.join(pastaV, nome);
    // Segurança: o caminho final precisa estar dentro da pasta do vendedor
    if (!caminhoArq.startsWith(pastaV + path.sep) || !fs.existsSync(caminhoArq) || !fs.statSync(caminhoArq).isFile()) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Arquivo não encontrado' }));
    }
    const mimeExt = {
      pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
      xls: 'application/vnd.ms-excel', xlsx: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      csv: 'text/csv', txt: 'text/plain', md: 'text/markdown', json: 'application/json',
      png: 'image/png', jpg: 'image/jpeg', jpeg: 'image/jpeg', webp: 'image/webp', gif: 'image/gif',
      mp4: 'video/mp4', mp3: 'audio/mpeg', wav: 'audio/wav', ogg: 'audio/ogg'
    };
    const extArq = (nome.split('.').pop() || '').toLowerCase();
    res.writeHead(200, {
      'Content-Type': mimeExt[extArq] || 'application/octet-stream',
      'Content-Disposition': 'attachment; filename="' + nome.replace(/"/g, '') + '"'
    });
    return res.end(fs.readFileSync(caminhoArq));
  }

  // GET /api/vendedores — lista todos os vendedores cadastrados (admin)
  if (method === 'GET' && rawUrl === '/api/vendedores') {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const dbV = readJSON(VENDEDORES_DB);
    return res.end(JSON.stringify(dbV));
  }

  // GET /api/vendedor/acesso/:id — página de acesso do cliente (vendedor treinado)
  // Requer token (?t=) para ver dados completos; sem token, mostra só status público.
  if (method === 'GET' && rawUrl.startsWith('/api/vendedor/acesso/')) {
    const rest = rawUrl.slice('/api/vendedor/acesso/'.length);
    const vendedorId = decodeURIComponent(rest.split('/')[0]);
    const urlP = new URL(req.url, 'http://' + (req.headers.host || 'localhost'));
    const token = urlP.searchParams.get('t') || '';
    const dbV = readJSON(VENDEDORES_DB);
    const v = dbV.find(x => x.id === vendedorId);
    if (!v) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Vendedor não encontrado' }));
    }
    if (v.status !== 'ativo') {
      res.writeHead(402, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Aguardando confirmação de pagamento' }));
    }
    let contexto = '';
    if (v.contextoArquivo && fs.existsSync(v.contextoArquivo)) {
      contexto = fs.readFileSync(v.contextoArquivo, 'utf8');
    }
    const dono = vendedor_ai.validarToken(v, token);
    const base = {
      ok: true,
      id: v.id,
      negocio: v.negocio,
      plano: v.plano,
      nomeBot: (v.personalidade && v.personalidade.nomeBot) || 'Severino',
      whatsapp: v.whatsapp || '',
      treinadoEm: v.treinadoEm || v.pagoEm || v.recebidoEm,
      contexto
    };
    if (dono) {
      Object.assign(base, {
        dono: true,
        email: v.email || '',
        ramo: v.ramo || '',
        redes: v.redes || '',
        produtos: v.produtos || [],
        treino: (v.treino && { texto: v.treino.texto || '', linkRef: v.treino.linkRef || '', arquivos: (v.treino.arquivos || []).map(a => ({ nome: a.nome, ext: a.ext, tamanho: a.tamanho })) }) || { texto: '', linkRef: '', arquivos: [] },
        personalidade: v.personalidade || {},
        canais: v.canais || [],
        qtdVendedores: v.qtdVendedores || 1,
        pagoEm: v.pagoEm || null,
        ativoAte: v.ativoAte || null,
        treinado: !!v.contextoTreinado
      });
    }
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify(base));
  }

  // PUT /api/vendedor/acesso/:id — comprador ajusta as informações do vendedor (token)
  if (method === 'PUT' && rawUrl.startsWith('/api/vendedor/acesso/')) {
    const rest = rawUrl.slice('/api/vendedor/acesso/'.length);
    const vendedorId = decodeURIComponent(rest.split('/')[0]);
    const urlP = new URL(req.url, 'http://' + (req.headers.host || 'localhost'));
    const token = urlP.searchParams.get('t') || '';
    const dbV = readJSON(VENDEDORES_DB);
    const v = dbV.find(x => x.id === vendedorId);
    if (!v || v.status !== 'ativo' || !vendedor_ai.validarToken(v, token)) {
      res.writeHead(403, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Acesso negado' }));
    }
    const dados = await parseBodyLimit(req, 120 * 1024 * 1024);
    try {
      if (dados.negocio !== undefined) v.negocio = String(dados.negocio).slice(0, 200);
      if (dados.ramo !== undefined) v.ramo = String(dados.ramo).slice(0, 100);
      if (dados.redes !== undefined) v.redes = String(dados.redes).slice(0, 500);
      if (dados.whatsapp !== undefined) v.whatsapp = String(dados.whatsapp).slice(0, 50);
      if (dados.email !== undefined && String(dados.email).includes('@')) v.email = String(dados.email).trim().toLowerCase();
      if (Array.isArray(dados.produtos)) v.produtos = dados.produtos.slice(0, 50);
      if (dados.treino) {
        if (!v.treino) v.treino = {};
        if (dados.treino.texto !== undefined) v.treino.texto = String(dados.treino.texto).slice(0, 50000);
        if (dados.treino.linkRef !== undefined) v.treino.linkRef = String(dados.treino.linkRef).slice(0, 2000);
      }
      if (dados.personalidade) {
        if (!v.personalidade) v.personalidade = {};
        Object.keys(dados.personalidade).forEach(k => {
          v.personalidade[k] = String(dados.personalidade[k]).slice(0, 2000);
        });
      }
      v.atualizadoEm = new Date().toISOString();
      v.contextoTreinado = false; // pede retreino
      writeJSON(VENDEDORES_DB, dbV);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true, message: 'Informações atualizadas. Retreine seu vendedor para aplicar.' }));
    } catch (e) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: e.message }));
    }
  }

  // POST /api/vendedor/acesso/:id/retreinar — regenera o contexto da IA (token)
  if (method === 'POST' && rawUrl.endsWith('/retreinar')) {
    const rest = rawUrl.slice('/api/vendedor/acesso/'.length).replace('/retreinar', '');
    const vendedorId = decodeURIComponent(rest);
    const urlP = new URL(req.url, 'http://' + (req.headers.host || 'localhost'));
    const token = urlP.searchParams.get('t') || '';
    const dbV = readJSON(VENDEDORES_DB);
    const v = dbV.find(x => x.id === vendedorId);
    if (!v || v.status !== 'ativo' || !vendedor_ai.validarToken(v, token)) {
      res.writeHead(403, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Acesso negado' }));
    }
    try {
      const treino = await vendedor_ai.treinar(v, VENDEDORES_DIR);
      v.contextoTreinado = true;
      v.contextoArquivo = treino.arquivo;
      v.treinadoEm = new Date().toISOString();
      writeJSON(VENDEDORES_DB, dbV);
      try {
        fs.appendFileSync(VENDEDORES_EVENTS, JSON.stringify({
          event: 'vendedor_retreinado', id: v.id, negocio: v.negocio, tokens: treino.tokens, timestamp: new Date().toISOString()
        }) + '\n');
      } catch (e) {}
      telegramBot.sendTelegram(process.env.TELEGRAM_CHAT_ID,
        '🔄 <b>Vendedor retreinado pelo comprador</b>\n\n🏪 ' + v.negocio + '\n👤 Bot: ' + ((v.personalidade && v.personalidade.nomeBot) || 'Severino') + '\n🤖 ' + treino.tokens + ' tokens',
        process.env.TELEGRAM_TOKEN);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true, tokens: treino.tokens, treinadoEm: v.treinadoEm, contexto: treino.contexto }));
    } catch (e) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: e.message }));
    }
  }

  // POST /api/vendedor/acesso/:id/chat — conversa com o vendedor treinado (token)
  if (method === 'POST' && rawUrl.endsWith('/chat')) {
    const rest = rawUrl.slice('/api/vendedor/acesso/'.length).replace('/chat', '');
    const vendedorId = decodeURIComponent(rest);
    const urlP = new URL(req.url, 'http://' + (req.headers.host || 'localhost'));
    const token = urlP.searchParams.get('t') || '';
    const dbV = readJSON(VENDEDORES_DB);
    const v = dbV.find(x => x.id === vendedorId);
    if (!v || v.status !== 'ativo' || !vendedor_ai.validarToken(v, token)) {
      res.writeHead(403, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Acesso negado' }));
    }
    const body = await parseBody(req);
    const pergunta = (body.pergunta || '').toString().slice(0, 2000);
    const historico = Array.isArray(body.historico) ? body.historico : [];
    if (!pergunta) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Pergunta vazia' }));
    }
    const r = await vendedor_ai.conversar(v, pergunta, historico, VENDEDORES_DIR);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, resposta: r.resposta, viaLLM: r.viaLLM }));
  }

  // POST /api/vendedor/status — atualiza status de um vendedor (admin)
  if (method === 'POST' && rawUrl === '/api/vendedor/status') {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const body = await parseBody(req);
    const dbV = readJSON(VENDEDORES_DB);
    const v = dbV.find(x => x.id === body.id);
    if (!v) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'Vendedor não encontrado' }));
    }
    v.status = body.status || v.status;
    writeJSON(VENDEDORES_DB, dbV);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, status: v.status }));
  }

  // GET /api/config/checkout — retorna links de pagamento Kiwify por plano/frequência
  if (method === 'GET' && rawUrl === '/api/config/checkout') {
    const checkout = {
      basico: { mensal: process.env.KIWIFY_BASICO_MENSAL || '', semestral: process.env.KIWIFY_BASICO_SEMESTRAL || '', anual: process.env.KIWIFY_BASICO_ANUAL || '' },
      pro: { mensal: process.env.KIWIFY_PRO_MENSAL || '', semestral: process.env.KIWIFY_PRO_SEMESTRAL || '', anual: process.env.KIWIFY_PRO_ANUAL || '' },
      empresa: { mensal: process.env.KIWIFY_EMPRESA_MENSAL || '', semestral: process.env.KIWIFY_EMPRESA_SEMESTRAL || '', anual: process.env.KIWIFY_EMPRESA_ANUAL || '' },
      avatar: process.env.KIWIFY_AVATAR || '',
      conteudo: process.env.KIWIFY_CONTEUDO || ''
    };
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, checkout }));
  }

  // POST /api/config/checkout — atualiza links (admin, protegido por SECRET_KEY)
  if (method === 'POST' && rawUrl === '/api/config/checkout') {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const body = await parseBody(req);
    const novos = body.checkout || {};
    const mapa = {
      'basico.mensal': 'KIWIFY_BASICO_MENSAL', 'basico.semestral': 'KIWIFY_BASICO_SEMESTRAL', 'basico.anual': 'KIWIFY_BASICO_ANUAL',
      'pro.mensal': 'KIWIFY_PRO_MENSAL', 'pro.semestral': 'KIWIFY_PRO_SEMESTRAL', 'pro.anual': 'KIWIFY_PRO_ANUAL',
      'empresa.mensal': 'KIWIFY_EMPRESA_MENSAL', 'empresa.semestral': 'KIWIFY_EMPRESA_SEMESTRAL', 'empresa.anual': 'KIWIFY_EMPRESA_ANUAL',
      'avatar': 'KIWIFY_AVATAR', 'conteudo': 'KIWIFY_CONTEUDO'
    };
    const envFile = path.join(__dirname, '.env');
    try {
      let env = fs.existsSync(envFile) ? fs.readFileSync(envFile, 'utf8') : '';
      const linhas = env.split('\n');
      for (const [chave, envVar] of Object.entries(mapa)) {
        const [planoKey, freqKey] = chave.split('.');
        let valor = chave.includes('.') ? (novos[planoKey] && novos[planoKey][freqKey]) || '' : novos[chave] || '';
        valor = String(valor || '').trim();
        if (valor) {
          const idx = linhas.findIndex(l => l.startsWith(envVar + '='));
          if (idx >= 0) linhas[idx] = envVar + '=' + valor;
          else linhas.push(envVar + '=' + valor);
          process.env[envVar] = valor;
        }
      }
      fs.writeFileSync(envFile, linhas.join('\n'));
      console.log('💳 Links Kiwify atualizados via admin');
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true }));
    } catch (e) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: e.message }));
    }
  }

  if (method === 'GET' && url === '/api/pendentes') {
    const db = readJSON(DB);
    const pendentes = db.filter(d => d.status === 'pendente');
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify(pendentes));
  }

  if (method === 'DELETE' && url.startsWith('/api/diagnostico/')) {
    const id = url.split('/api/diagnostico/')[1];
    const db = readJSON(DB);
    const idx = db.findIndex(d => d.id === id);
    if (idx >= 0) {
      db.splice(idx, 1);
      writeJSON(DB, db);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true }));
    }
    res.writeHead(404, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: false }));
  }

  if (method === 'DELETE' && url === '/api/diagnosticos') {
    writeJSON(DB, []);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true }));
  }

  // POST /api/lead — captura lead pelo site (e-mail + Telegram)
  if (method === 'POST' && rawUrl === '/api/lead') {
    const body = await parseBody(req);
    const email = (body.email || '').toString().trim().toLowerCase();
    const telegram = (body.telegram || '').toString().trim().replace(/^@/, '');
    const nome = (body.nome || '').toString().trim();
    if (!email.includes('@') || !email.includes('.')) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: false, error: 'E-mail inválido' }));
    }
    const LEADS_PATH = path.join(__dirname, 'ecosystem', 'leads.json');
    if (!fs.existsSync(LEADS_PATH)) writeJSON(LEADS_PATH, []);
    const leads = readJSON(LEADS_PATH);
    const idx = leads.findIndex(l => l.email && String(l.email).toLowerCase() === email);
    if (idx >= 0) {
      leads[idx].telegram_username = telegram || leads[idx].telegram_username;
      leads[idx].fonte = 'site';
      leads[idx].updatedAt = new Date().toISOString();
    } else {
      leads.push({
        id: Date.now().toString(36) + Math.random().toString(36).slice(2, 6),
        nome,
        email,
        telegram_username: telegram,
        fonte: 'site',
        status: 'novo',
        score: 1,
        createdAt: new Date().toISOString()
      });
    }
    writeJSON(LEADS_PATH, leads);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, email, telegram: telegram || '' }));
  }



  // ========== SISTEMA DE LICENÇAS 7+7+FREE ==========
  const LICENSE_PATH = path.join(DATA_DIR, 'licenses.json');
  if (!fs.existsSync(LICENSE_PATH)) writeJSON(LICENSE_PATH, {});

  function loadLicenses() { return readJSON(LICENSE_PATH); }

  function generateCode() {
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    let code = 'VIP7-';
    for (let i = 0; i < 6; i++) code += chars[Math.floor(Math.random() * chars.length)];
    return code;
  }

  function getTier(activatedAt) {
    if (!activatedAt) return 'pending';
    const daysSince = (Date.now() - new Date(activatedAt).getTime()) / 86400000;
    if (daysSince <= 7) return 'vip';
    return 'basic';
  }

  // Resolve telegramId/nome de um código vinculado via /vincular ou /resgatar
  function telegramDoCode(code) {
    try {
      const nf = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'notifications.json'), 'utf8'));
      for (const [chatId, u] of Object.entries(nf)) {
        if (u && u.licenseCode === code) return { telegramId: chatId, nome: u.nome || '' };
      }
    } catch {}
    return {};
  }

  // Posta trial.ativado no orquestrador (porta 3335) para o retentor seguir o lead
  function emitirTrialAtivado({ code, email = '', nome = '', telegramId = null, source = '' }) {
    try {
      const extra = telegramId ? {} : telegramDoCode(code);
      const chatId = telegramId || extra.telegramId || null;
      const nomeFinal = nome || extra.nome || (email ? email.split('@')[0] : 'trader');
      const payload = JSON.stringify({
        tipo: 'trial.ativado',
        origem: 'servidor:' + (source || 'ativacao'),
        produto: 'btcweather',
        payload: {
          leadId: 'lic-' + code,
          nome: nomeFinal,
          telegramId: chatId,
          email,
          produto: 'btcweather',
          code,
          origem: source || 'ativacao'
        }
      });
      const req = http.request({
        hostname: '127.0.0.1',
        port: parseInt(process.env.ECOSISTEMA_PORT || '3335', 10),
        path: '/api/ecosystem/enviar',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Admin-Secret': process.env.SECRET_KEY || '',
          'Content-Length': Buffer.byteLength(payload)
        }
      }, (res) => { res.resume(); });
      req.on('error', (e) => console.error('❌ emitirTrialAtivado:', e.message));
      req.write(payload);
      req.end();
    } catch (e) {
      console.error('❌ emitirTrialAtivado:', e.message);
    }
  }

  // Posta venda.confirmada no orquestrador para o retentor/cobrador marcar o lead como pago
  function emitirVendaConfirmada({ code, email = '', nome = '', value = 0 }) {
    try {
      const payload = JSON.stringify({
        tipo: 'venda.confirmada',
        origem: 'servidor:kiwify',
        produto: 'btcweather',
        payload: {
          leadId: 'lic-' + code,
          customer_email: email,
          customer_name: nome,
          value,
          code,
          produto: 'btcweather'
        }
      });
      const req = http.request({
        hostname: '127.0.0.1',
        port: parseInt(process.env.ECOSISTEMA_PORT || '3335', 10),
        path: '/api/ecosystem/enviar',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Admin-Secret': process.env.SECRET_KEY || '',
          'Content-Length': Buffer.byteLength(payload)
        }
      }, (res) => { res.resume(); });
      req.on('error', (e) => console.error('❌ emitirVendaConfirmada:', e.message));
      req.write(payload);
      req.end();
    } catch (e) {
      console.error('❌ emitirVendaConfirmada:', e.message);
    }
  }

  // POST /api/license/generate — gera N códigos VIP7
  if (method === 'POST' && rawUrl === '/api/license/generate') {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const body = await parseBody(req);
    const count = Math.min(Math.max(parseInt(body.count) || 1, 1), 50);
    const source = body.source || 'api';
    const licenses = loadLicenses();
    const codes = [];
    for (let i = 0; i < count; i++) {
      let code = generateCode();
      while (licenses[code]) code = generateCode();
      licenses[code] = { createdAt: new Date().toISOString(), activatedAt: null, source };
      codes.push(code);
    }
    writeJSON(LICENSE_PATH, licenses);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, codes }));
  }

  // GET /api/license/validate?code=XXX — valida e retorna tier
  if (method === 'GET' && rawUrl === '/api/license/validate') {
    const urlObj = new URL(req.url, `http://${req.headers.host}`);
    const code = urlObj.searchParams.get('code');
    if (!code) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'code required' }));
    }
    const licenses = loadLicenses();
    const license = licenses[code];
    if (!license) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ valid: false, error: 'Código inválido' }));
    }
    const tier = getTier(license.activatedAt);
    const activatedAt = license.activatedAt;
    const daysLeft = activatedAt ? Math.max(0, 7 - Math.floor((Date.now() - new Date(activatedAt).getTime()) / 86400000)) : 7;
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ valid: true, code, tier, daysLeft, activatedAt: !!activatedAt, createdAt: license.createdAt }));
  }

  // POST /api/license/activate — ativa um código
  if (method === 'POST' && rawUrl === '/api/license/activate') {
    const body = await parseBody(req);
    const code = body.code;
    if (!code) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'code required' }));
    }
    const licenses = loadLicenses();
    const license = licenses[code];
    if (!license) {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Código inválido' }));
    }
    if (license.activatedAt) {
      res.writeHead(409, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Código já ativado', activatedAt: license.activatedAt }));
    }
    license.activatedAt = new Date().toISOString();
    writeJSON(LICENSE_PATH, licenses);
    emitirTrialAtivado({ code, email: license.customerEmail || '', nome: license.customerName || '', source: license.source || 'activate' });
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, code, activatedAt: license.activatedAt, tier: 'vip' }));
  }

  // POST /api/trial — teste grátis 7 dias (público, protegido pelo rate limit)
  if (method === 'POST' && rawUrl === '/api/trial') {
    const body = await parseBody(req);
    const licenses = loadLicenses();
    let code = generateCode();
    while (licenses[code]) code = generateCode();
    const source = body.source || 'frontend-offer';
    const email = String(body.email || '').trim().toLowerCase();
    const nome = String(body.nome || '').trim();
    const telegram = body.telegram || '';
    const lic = { createdAt: new Date().toISOString(), activatedAt: new Date().toISOString(), source };
    if (email) lic.customerEmail = email;
    if (nome) lic.customerName = nome;
    if (telegram) lic.telegram = telegram;
    licenses[code] = lic;
    writeJSON(LICENSE_PATH, licenses);
    // Chamadas internas do ecossistema (onboarder) já postam trial.ativado — não duplicar.
    if (req.headers['x-ecosystem'] !== '1') {
      emitirTrialAtivado({ code, email, nome, source });
    }
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, code, tier: 'vip', daysLeft: 7 }));
  }

  // ========== EVENT LOG + COHORT COUNTER ==========
  const EVENTS_LOG = path.join(DATA_DIR, 'events.jsonl');
  const COHORT_PATH = path.join(DATA_DIR, 'pricing-state.json');
  if (!fs.existsSync(COHORT_PATH)) writeJSON(COHORT_PATH, { totalSignups: 0, totalActivations: 0, cohorts: [] });

  // POST /api/event — registra evento no JSONL
  if (method === 'POST' && rawUrl === '/api/event') {
    const body = await parseBody(req);
    const event = { event: body.event || 'unknown', timestamp: new Date().toISOString(), ...body };
    delete event.secret;
    try {
      fs.appendFileSync(EVENTS_LOG, JSON.stringify(event) + '\n');
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true }));
    } catch (e) {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: e.message }));
    }
  }

  // GET /api/cohort — retorna estado do contador de coorte
  if (method === 'GET' && rawUrl === '/api/cohort') {
    const state = readJSON(COHORT_PATH);
    const discountTier = state.totalSignups <= 100 ? 0 : state.totalSignups <= 200 ? 1 : state.totalSignups <= 300 ? 2 : 3;
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({
      totalSignups: state.totalSignups,
      totalActivations: state.totalActivations,
      discountTier,
      slotsLeft: Math.max(0, 100 - state.totalSignups),
      discounts: {
        foundingMember: state.totalSignups < 100 ? 40 : state.totalSignups < 200 ? 35 : state.totalSignups < 300 ? 30 : 25,
        annualExtra: 10
      }
    }));
  }

  // POST /api/cohort/increment — incrementa contador (autenticado)
  if (method === 'POST' && rawUrl === '/api/cohort/increment') {
    const adminSecret = req.headers['x-admin-secret'];
    if (adminSecret !== process.env.SECRET_KEY) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }
    const body = await parseBody(req);
    const field = body.field === 'activation' ? 'totalActivations' : 'totalSignups';
    const state = readJSON(COHORT_PATH);
    state[field] = (state[field] || 0) + 1;
    writeJSON(COHORT_PATH, state);
    res.writeHead(200, { 'Content-Type': 'application/json' });
    return res.end(JSON.stringify({ ok: true, [field]: state[field] }));
  }



  // POST /api/purchase/webhook
  if (method === 'POST' && rawUrl.startsWith('/api/purchase/webhook')) {
    const body = await parseBody(req);

    // 🔒 Autenticação: exige o secret (via query ?secret=, header X-Webhook-Secret/X-Kiwify-Token ou body.secret)
    // para impedir que qualquer pessoa gere códigos VIP pela URL pública.
    const expectedSecret = process.env.SECRET_KEY || '';
    const urlObj = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
    const providedSecret = urlObj.searchParams.get('secret') || req.headers['x-webhook-secret'] || req.headers['x-kiwify-token'] || body.secret || '';
    if (!expectedSecret || providedSecret !== expectedSecret) {
      res.writeHead(401, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Unauthorized' }));
    }

    // Normaliza payload que venha aninhado em "data" (ex.: algumas integrações)
    if (body.data && typeof body.data === 'object') {
      Object.assign(body, body.data);
    }

    // Payload vazio/sem dados (ex.: botão "Testar Webhook" da Kiwify): responde OK sem gerar código
    const hasOrderData = body.event || body.order_id || body.transaction || body.id || body.customer || body.customer_email || body.email || body.purchase || body.order_status || body.product;
    if (!hasOrderData) {
      console.log('💓 Webhook recebido sem dados (heartbeat/teste)');
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true, heartbeat: true }));
    }

    // Filtra eventos: aprovação de pagamento = gerar código; eventos de não-pagamento = ignorar.
    // Eventos da Kiwify são em pt-BR (compra_aprovada, compra_reembolsada, etc.).
    const PAID_EVENTS = ['compra_aprovada', 'order.paid', 'order.payment_confirmed', 'purchase_approved', 'payment_confirmed', 'payment_received'];
    const IGNORED_EVENTS = ['compra_reembolsada', 'compra_recusada', 'compra_recusado', 'chargeback', 'boleto_gerado', 'pix_gerado', 'carrinho_abandonado', 'subscription_canceled', 'subscription_late'];
    const evt = (body.event || '').toLowerCase();
    if (evt) {
      if (IGNORED_EVENTS.includes(evt)) {
        console.log(`🚫 Webhook ignorado (evento ${evt})`);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: true, ignored: evt }));
      }
      if (!PAID_EVENTS.includes(evt)) {
        // Evento desconhecido: se houver order_status explícito não-'paid', ignora; senão trata como compra.
        const st = (body.order_status || '').toLowerCase();
        if (st && st !== 'paid' && st !== 'approved' && st !== 'completed') {
          console.log(`🚫 Webhook ignorado (evento ${evt}, status ${st})`);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          return res.end(JSON.stringify({ ok: true, ignored: evt }));
        }
      }
    } else if (body.order_status && !['paid', 'approved', 'completed'].includes(String(body.order_status).toLowerCase())) {
      console.log(`🚫 Webhook ignorado (order_status ${body.order_status})`);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ ok: true, ignored: body.order_status }));
    }

    const gateway = body.gateway || (req.headers['x-gateway'] || 'kiwify');
    let customerName = body.customer_name || body.name || body.nome || '';
    if (body.customer && body.customer.full_name) customerName = body.customer.full_name;
    if (body.buyer && body.buyer.name) customerName = body.buyer.name;
    if (body.kiwify && body.kiwify.customer_name) customerName = body.kiwify.customer_name;
    let customerEmail = body.customer_email || body.email || '';
    if (body.customer && body.customer.email) customerEmail = body.customer.email;
    if (body.buyer && body.buyer.email) customerEmail = body.buyer.email;
    if (body.kiwify && body.kiwify.customer_email) customerEmail = body.kiwify.customer_email;
    let productName = body.product_name || body.produto || '';
    if (body.product && body.product.product_name) productName = body.product.product_name;
    if (body.kiwify && body.kiwify.product_name) productName = body.kiwify.product_name;
    const plan = body.plan || productName || 'vip_mensal';
    let value = body.value || body.price || body.amount || 0;
    if (body.purchase && body.purchase.original_offer_price) value = body.purchase.original_offer_price;
    // Kiwify envia valores em CENTAVOS (ex.: 4700 = R$47,00) — converte para reais
    if (gateway === 'kiwify' && value > 0 && value < 100000) value = value / 100;
    let tx = body.transaction || body.id || body.transaction_id || '';
    if (body.order_id) tx = body.order_id;
    if (body.kiwify && body.kiwify.id) tx = body.kiwify.id;
    if (!customerEmail && !tx) {
      res.writeHead(400, {'Content-Type':'application/json'});
      return res.end(JSON.stringify({error:'customer_email or transaction id required'}));
    }

    // ===== SEVERINO VENDEDOR IA — ativação 100% automática (sem humano) =====
    // Identifica o produto do Severino pelo nome/plano; se for, ativa o vendedor,
    // treina a IA com os materiais e entrega o acesso ao cliente.
    const ehSeverino = /severino|vendedor|consul|basico|básico|pro/.test(String(productName || plan || '').toLowerCase());
    if (ehSeverino) {
      try {
        const vendedoresDir = path.join(DATA_DIR, 'vendedores');
        let dbV = [];
        try { dbV = JSON.parse(fs.readFileSync(path.join(vendedoresDir, 'vendedores.json'), 'utf8')); } catch (e) {}
        const emailNorm = String(customerEmail || '').trim().toLowerCase();
        const vendedor = dbV.find(v =>
          (v.email && String(v.email).toLowerCase() === emailNorm) ||
          (v.whatsapp && customerEmail && String(v.whatsapp).replace(/\D/g, '') === String(customerEmail).replace(/\D/g, ''))
        );

        if (!vendedor) {
          console.log('🚫 Severino: compra sem cadastro prévio (email ' + (emailNorm || '?') + ') — aguardando cadastro');
          // Registra a compra como pendente de cadastro para o lead ser chamado de volta
          const esperaPath = path.join(DATA_DIR, 'vendedores', 'compra_sem_cadastro.json');
          let espera = [];
          try { espera = JSON.parse(fs.readFileSync(esperaPath, 'utf8')); } catch (e) {}
          espera.push({ email: emailNorm, nome: customerName, plano: plan, valor: value, tx, criadoEm: new Date().toISOString() });
          try { fs.writeFileSync(esperaPath, JSON.stringify(espera, null, 2)); } catch (e) {}
          telegramBot.sendTelegram(process.env.TELEGRAM_CHAT_ID,
            '🛒 <b>SEVERINO: compra sem cadastro!</b>\n\n👤 ' + (customerName || '—') + '\n📧 ' + (emailNorm || '—') + '\n📦 ' + plan + '\n💰 R$ ' + value + '\n🧾 ' + tx + '\n\n⚠️ Cliente pagou mas ainda não fez o cadastro do vendedor.', process.env.TELEGRAM_TOKEN);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          return res.end(JSON.stringify({ ok: true, status: 'aguardando_cadastro', product: 'severino' }));
        }

        // Ativação automática
        vendedor.status = 'ativo';
        vendedor.pagoEm = new Date().toISOString();
        vendedor.transactionId = tx;
        vendedor.valorPago = value;
        vendedor.ativoAte = new Date(Date.now() + (vendedor.frequencia === 'anual' ? 365 : vendedor.frequencia === 'semestral' ? 180 : 30) * 86400000).toISOString();

        // Treinamento da IA com os materiais
        const treino = await vendedor_ai.treinar(vendedor, vendedoresDir);
        vendedor.contextoTreinado = true;
        vendedor.contextoArquivo = treino.arquivo;
        vendedor.treinadoEm = new Date().toISOString();

        try { fs.writeFileSync(path.join(vendedoresDir, 'vendedores.json'), JSON.stringify(dbV, null, 2)); } catch (e) {}
        try {
          fs.appendFileSync(VENDEDORES_EVENTS, JSON.stringify({
            event: 'vendedor_ativo', id: vendedor.id, negocio: vendedor.negocio, plano: vendedor.plano,
            valor: value, tx, tokens: treino.tokens, timestamp: new Date().toISOString()
          }) + '\n');
        } catch (e) {}

        const link = vendedor_ai.linkAcesso(vendedor);
        console.log('🤖 SEVERINO ATIVADO AUTOMATICAMENTE:', vendedor.id, '—', vendedor.negocio, '| plano:', vendedor.plano, '| tokens:', treino.tokens);

        // Entrega automática ao admin + cliente
        telegramBot.sendTelegram(process.env.TELEGRAM_CHAT_ID,
          '✅ <b>SEVERINO PAGO E ATIVADO (automático)</b>\n\n' +
          '🏪 ' + vendedor.negocio + '\n👤 Bot: ' + ((vendedor.personalidade && vendedor.personalidade.nomeBot) || 'Severino') + '\n' +
          '📦 Plano: ' + vendedor.plano + ' | 💰 R$ ' + value + '\n📧 ' + (emailNorm || '—') + '\n🧾 ' + tx + '\n' +
          '🤖 IA treinada (' + treino.tokens + ' tokens)\n🔗 Acesso: ' + link,
          process.env.TELEGRAM_TOKEN);

        // Boas-vindas ao comprador no Telegram (se vinculado) — mesma lógica do BTC
        if (customerEmail && process.env.TELEGRAM_TOKEN) {
          try {
            const notifPath = path.join(DATA_DIR, 'notifications.json');
            const notifUsers = fs.existsSync(notifPath) ? JSON.parse(fs.readFileSync(notifPath, 'utf8')) : {};
            const buyer = Object.values(notifUsers).find(u => u.email && String(u.email).toLowerCase() === emailNorm);
            if (buyer && buyer.telegramId) {
              telegramBot.sendTelegram(buyer.telegramId,
                '🎉 <b>Seu Vendedor IA está no ar!</b>\n\n' +
                '🏪 ' + vendedor.negocio + '\n👤 Bot: ' + ((vendedor.personalidade && vendedor.personalidade.nomeBot) || 'Severino') + '\n📦 Plano: ' + vendedor.plano + '\n\n' +
                '🔗 <b>Seu acesso:</b> ' + link + '\n\n' +
                'Teste seu vendedor e comece a vender!', process.env.TELEGRAM_TOKEN);
            }
          } catch (e) { console.error('❌ Erro na entrega por Telegram:', e.message); }
        }

        res.writeHead(200, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: true, status: 'ativo', product: 'severino', vendedorId: vendedor.id, link }));
      } catch (e) {
        console.error('❌ Erro na ativação automática do Severino:', e.message);
        res.writeHead(500, { 'Content-Type': 'application/json' });
        return res.end(JSON.stringify({ ok: false, error: 'Erro na ativação automática: ' + e.message }));
      }
    }
    const licenses = loadLicenses();
    let code = generateCode();
    while (licenses[code]) code = generateCode();
    licenses[code] = {
      createdAt: new Date().toISOString(),
      activatedAt: null,
      source: 'purchase:' + gateway,
      plan: plan,
      customerName: customerName,
      customerEmail: customerEmail,
      transactionId: tx,
      value: value
    };
    writeJSON(LICENSE_PATH, licenses);
    const purchasesPath = path.join(DATA_DIR, 'purchases.json');
    let purchases = [];
    try { purchases = JSON.parse(fs.readFileSync(purchasesPath, 'utf8')); } catch (e) {}
    purchases.push({
      transactionId: tx,
      gateway: gateway,
      customerName: customerName,
      customerEmail: customerEmail,
      plan: plan,
      value: value,
      code: code,
      createdAt: new Date().toISOString()
    });
    writeJSON(purchasesPath, purchases);

    emitirVendaConfirmada({ code, email: customerEmail, nome: customerName, value });

    // ===== Entrega do código ao comprador =====
    const tgToken = process.env.TELEGRAM_TOKEN || '';
    const adminChat = process.env.TELEGRAM_CHAT_ID || '';
    const linkAtivar = 'https://btcweatherpanel.com/btc-weather-panel/?code=' + code;
    const nomeComprador = customerName || 'Cliente';
    const msgResumo = `🛒 <b>NOVA COMPRA</b>\n\n👤 Cliente: ${nomeComprador}\n📧 Email: ${customerEmail || '—'}\n📦 Plano: ${plan}\n💰 Valor: R$ ${value}\n🧾 Tx: ${tx}\n\n🔑 Código: <code>${code}</code>\n🌐 Ativar: ${linkAtivar}`;

    // 1. Avisar o admin (nunca perder uma venda silenciosamente)
    if (tgToken && adminChat) {
      telegramBot.sendTelegram(adminChat, msgResumo, tgToken);
    }

    // 2. Mensagem automática de boas-vindas ao comprador (se ele tiver Telegram vinculado ao mesmo email)
    if (customerEmail && tgToken) {
      try {
        const notifPath = path.join(DATA_DIR, 'notifications.json');
        const notifUsers = fs.existsSync(notifPath) ? JSON.parse(fs.readFileSync(notifPath, 'utf8')) : {};
        const buyer = Object.values(notifUsers).find(u => u.email && String(u.email).toLowerCase() === String(customerEmail).toLowerCase());
        if (buyer && buyer.telegramId) {
          const msgWelcome = `🎉 <b>Bem-vindo(a) ao BTC Weather Panel, ${nomeComprador}!</b>\n\nSeu acesso VIP está liberado:\n\n🔑 <b>Código:</b> <code>${code}</code>\n📦 <b>Plano:</b> ${plan}\n\n━━━━━━━━━━━━━━━━━━━━━━\n\n✅ <b>Passo 1 — Ative seu acesso:</b>\n${linkAtivar}\n\n💬 <b>Passo 2 — Receba suas notificações:</b>\nEnvie aqui /vincular ${code} e ative:\n• 📊 Análise diária do BTC às 9h\n• 🌦️ Alertas de mudança de clima\n• 📡 Sinais operacionais\n\n📋 <b>Passo 3 — Confira:</b>\nUse /status para ver seu VIP.\n\n❓ Precisa de ajuda? Envie /ajuda.`;
          telegramBot.sendTelegram(buyer.telegramId, msgWelcome, tgToken);
          console.log(`💌 Boas-vindas enviadas ao comprador ${customerEmail}`);
        }
      } catch (e) {
        console.error('❌ Erro na entrega do código por Telegram:', e.message);
      }
    }

    res.writeHead(200, {'Content-Type':'application/json'});
    return res.end(JSON.stringify({ok:true, code:code, customerName:customerName, customerEmail:customerEmail, plan:plan}));
  }

    // ========== ROBOTS.TXT ==========
  if (method === 'GET' && rawUrl === '/robots.txt') {
    const robotsTxt = `User-agent: *
Allow: /btc-weather-panel/
Allow: /consultor
Allow: /venda
Allow: /atendimento
Allow: /agendabarber
Allow: /transparencia
Disallow: /app/
Disallow: /painel/
Disallow: /api/
Disallow: /ecosistema/

Sitemap: https://severinobot.com/sitemap.xml`;
    res.writeHead(200, { 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'public, max-age=86400' });
    return res.end(robotsTxt);
  }

  // ========== SITEMAP.XML ==========
  if (method === 'GET' && rawUrl === '/sitemap.xml') {
    const hoje = new Date().toISOString().slice(0, 10);
    const xml = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    <loc>https://severinobot.com/btc-weather-panel/</loc>
    <lastmod>${hoje}</lastmod>
    <changefreq>daily</changefreq>
    <priority>1.0</priority>
  </url>
  <url>
    <loc>https://severinobot.com/consultor</loc>
    <lastmod>${hoje}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.8</priority>
  </url>
  <url>
    <loc>https://severinobot.com/transparencia</loc>
    <lastmod>${hoje}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.6</priority>
  </url>
  <url>
    <loc>https://severinobot.com/agendabarber/</loc>
    <lastmod>${hoje}</lastmod>
    <changefreq>weekly</changefreq>
    <priority>0.5</priority>
  </url>
</urlset>`;
    res.writeHead(200, { 'Content-Type': 'application/xml; charset=utf-8', 'Cache-Control': 'public, max-age=86400' });
    return res.end(xml);
  }

  // ========== ARQUIVOS ESTÁTICOS ==========
  if (rawUrl.startsWith('/weather-panel') || rawUrl.startsWith('/btc-weather-panel')) {
    // HTML is served with html{display:none} to hide cached content until fresh page loads
let relativePath = 'index.html';

    if (url !== '/weather-panel' && url !== '/btc-weather-panel' && url !== '/weather-panel/' && url !== '/btc-weather-panel/') {
      relativePath = url.replace('/weather-panel/', '').replace('/btc-weather-panel/', '');
    }
    const weatherRoot = process.platform === 'win32' ? 'C:\\Users\\3\\Ted\\btc-weather-panel' : '/root/btc-weather-panel';
    const weatherPath = path.join(weatherRoot, relativePath);
    if (fs.existsSync(weatherPath)) {
      const extname = path.extname(weatherPath).toLowerCase();
      res.writeHead(200, { 
        'Content-Type': mime[extname] || 'application/octet-stream',
        'Cache-Control': 'no-store, no-cache, must-revalidate, proxy-revalidate',
        'Pragma': 'no-cache',
        'Expires': '0',
        'Surrogate-Control': 'no-store'
      });
      return res.end(fs.readFileSync(weatherPath));
    }
  }

  // Outros arquivos estáticos
  // Root redirect to BTC Weather Panel
  var filePath;

  if (url === '/') {
    // Domínio dedicado do Severino → exibe a página do Vendedor IA na raiz
    const host = (req.headers.host || '').toLowerCase();
    if (host === 'severinobot.com' || host === 'www.severinobot.com') {
      filePath = path.join(ROOT, 'consultor.html');
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return res.end(fs.readFileSync(filePath));
    }
    if (host === 'app.btcweatherpanel.com') {
      filePath = path.join(ROOT, 'app.html');
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      return res.end(fs.readFileSync(filePath));
    }
    res.writeHead(302, { 'Location': '/btc-weather-panel/' });
    return res.end();
  }

  // Severino Consultor IA at /severino-ia/
  if (url.startsWith('/severino-ia')) {
    let rest = url.slice('/severino-ia'.length);
    if (rest === '' || rest === '/') rest = '';
    const sevPath = rest || '/consultor.html';
    filePath = path.join(ROOT, sevPath);
    const ext3 = path.extname(filePath).toLowerCase();
    if (fs.existsSync(filePath)) {
      res.writeHead(200, { 'Content-Type': mime[ext3] || 'text/html; charset=utf-8' });
      return res.end(fs.readFileSync(filePath));
    }
    res.writeHead(404, { 'Content-Type': 'text/plain' });
    return res.end('404');
  }

  filePath = path.join(ROOT, url);
  const ext = path.extname(filePath).toLowerCase();

  fs.readFile(filePath, (err, data) => {
    if (err) {
      // Tenta da raiz do projeto
      filePath = path.join(__dirname, url === '/' ? 'index.html' : url);
      fs.readFile(filePath, (err2, data2) => {
        if (err2) {
          res.writeHead(404, { 'Content-Type': 'text/plain' });
          res.end('404');
        } else {
          const ext2 = path.extname(filePath).toLowerCase();
          res.writeHead(200, { 'Content-Type': mime[ext2] || 'application/octet-stream' });
          res.end(data2);
        }
      });
    } else {
      res.writeHead(200, { 'Content-Type': mime[ext] || 'application/octet-stream' });
      res.end(data);
    }
  });
}).listen(PORT, () => {
  console.log('🦾 Severino Consultor — Servidor Principal');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log(`🌐 http://localhost:${PORT}/ → Redireciona para /btc-weather-panel/`);
  console.log(`🦾 http://localhost:${PORT}/severino-ia/ → Severino Consultor IA`);
  console.log(`📡 API Licenças: /api/license/*`);
  console.log(`📡 API Eventos: /api/event | /api/cohort`);
  console.log(`🤖 Telegram Bot: @btcweatherpanel_bot`);
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
});