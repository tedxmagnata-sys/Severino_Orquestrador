/**
 * Serviço de IA do LeadBook Engine (OpenRouter) — zero dependências.
...
function modelos() {
  return {
    ...
    forte: env('LLM_MODEL_FORTE', 'openai/gpt-4o-mini')
  };
}
...
async function perguntar({ sistema = '', mensagens = [], modelo = 'barato', temperatura = 0.7, maxTokens, agente = 'leadbook', ignorarOrcamento = false } = {}) {
  ...
  if (!key) throw new Error('LLM_API_KEY ausente — configure no .env (na pasta do app: leadbook/.env)');
    ...
    try {
      const resp = await fetch(BASE, {
        ...
        headers: {
          ...
          'HTTP-Referer': env('LLM_REFERER', 'https://leadbook.app'),
          'X-Title': 'LeadBook Engine'
        },
      ...
      if (!conteudo && msg.reasoning) conteudo = msg.reasoning;
